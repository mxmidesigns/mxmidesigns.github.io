...
function getMixpanelID() {
    var mixpanelID = window.localStorage.getItem("mixpanel_id");
    if (!mixpanelID) {
        mixpanelID = analyticsUserIdGenerator();
        window.localStorage.setItem('mixpanel_id', mixpanelID);
    }
...
function initializeHrefListener(currentPage) {
    var startTime = new Date();
    document.body.onclick = function(e){
        e = e || event;
        var from = findParent('a', e.target || e.srcElement);
        if (from && e.srcElement){
            e.preventDefault();
            var mixpanel = getMixpanelID();
            ...
            mixpanel.track("herf" , {
                ...
                timeOnPage: millisToMinutesAndSeconds(endTime.getTime() - startTime.getTime())
            });
            if (href) {
